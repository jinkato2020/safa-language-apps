// App A ja「方→ほう」5件を再生成(SSML <sub>適用) → audio/ja/{conv,gram}/<id>.mp3
//  ja=Neural2-B@0.95。読みは scripts/_newthemes_ssml.json(ja)。
...
const SSML_JA = (rd(path.join(ROOT, 'scripts/_newthemes_ssml.json')).ja) || {};
...
const IDS = ['1-3-16', '10-3-8', '14-3-2', '15-3-7', '22-12'];
let ok = 0;
for (const id of IDS) {
  ...
  console.log(`OK ${mode}/${id} ${ssml ? '[SSML]' : '[plain]'} 「${text.slice(0, 22)}」 ${buf.length}B`); ok++;
}
console.log(`\n再生成完了: ${ok}/${IDS.length}`);
