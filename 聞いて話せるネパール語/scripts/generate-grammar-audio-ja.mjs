...
// 例:
//   1シート目の A2  → 1-1.mp3
//   30シート目の A21 → 30-20.mp3
//
// 既存ファイルはスキップするので、中断後に再実行すれば続きから処理可能。
//
// 実行: npm run generate-grammar-audio-ja
...
const XLSX_PATH = path.join(PROJECT_ROOT, '文法.xlsx');
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'japanese-grammar');
...
async function main() {
  ...
  for (let s = 0; s < sheets.length; s++) {
    ...
    for (let r = 0; r < 20; r++) {
      ...
      const fileName = `${sheetIdx}-${exampleIdx}.mp3`;
      ...
      const text = getCellText(sheet.getRow(rowNum).getCell(1)); // A列 = 日本語
      if (!text) {
  ...
  if (failures.length > 0) {
    ...
    console.log('\n再実行 (npm run generate-grammar-audio-ja) で失敗分のみ再試行できます。');
  }
}
