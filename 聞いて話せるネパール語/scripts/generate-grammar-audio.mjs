...
//
// 実行: npm run generate-grammar-audio
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'nepali-grammar');
...
async function main() {
  ...
  for (let s = 0; s < sheets.length; s++) {
    ...
    for (let r = 0; r < 20; r++) {
      ...
      const text = getCellText(sheet.getRow(rowNum).getCell(2));
      if (!text) {
  ...
  if (failures.length > 0) {
    ...
    console.log('\n再実行 (npm run generate-grammar-audio) で失敗分のみ再試行できます。');
  }
}
