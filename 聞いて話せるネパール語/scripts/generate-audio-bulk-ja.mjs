...
//
// 実行: npm run generate-audio-bulk-ja
...
// A/C/E 列(1-indexed)に対応するレベル番号
const LEVEL_COLUMNS = [
  { col: 1, levelIdx: 1, label: '初級' },
  { col: 3, levelIdx: 2, label: '中級' },
  { col: 5, levelIdx: 3, label: '上級' },
];
...
async function main() {
  ...
  if (failures.length > 0) {
    ...
    console.log('\n再実行 (npm run generate-audio-bulk-ja) で失敗分のみ再試行できます。');
  }
}
