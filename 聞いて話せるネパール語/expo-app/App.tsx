...
import { Animated, Image, Pressable, StyleSheet, useColorScheme, View } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import Svg, { Circle, Line, Path, Rect } from 'react-native-svg';
import { setAudioModeAsync } from 'expo-audio';
import { useVideoPlayer, VideoView } from 'expo-video';

import ListenSourceScreen from './src/screens/ListenSourceScreen';
...
// ─── タブアイコン ────────────────────────────
function TabIcon({ children, color, focused }: { children: React.ReactNode; color: string; focused: boolean }) {
  return (
...
function VideoSplash({ onDone }: { onDone: () => void }) {
  const player = useVideoPlayer(require('./assets/safa-splash.mp4'), p => {
    p.loop = false;
    // 動画プレイヤーが iOS の音声セッションを排他的に奪わないようにする
    // ※ これをしないと、動画終了後に聞き流しモードの音声が再生されなくなる
    p.audioMixingMode = 'mixWithOthers';
    // 念のためミュート（スプラッシュ動画に音声は不要）
    p.muted = true;
    p.play();
  });
  ...
  useEffect(() => {
    const handleDone = async () => {
      // 動画終了後、音声セッションを expo-audio 用に再設定
      try {
...
export default function App() {
  ...
  // iOSのサイレントモードでも音声が再生されるよう、アプリ起動時に設定
  useEffect(() => {
    setAudioModeAsync({
      playsInSilentMode: true,
      shouldPlayInBackground: true,
  ...
  if (!splashDone) {
    return <VideoSplash onDone={() => setSplashDone(true)} />;
  }
