// App B ベトナム語パック 辞書(全件・最高品質): 日本語文を分解し各語にベトナム語の意味+読み+品詞+注記。
//  出力: overlays/vi.json の convVocab(examples.json jp) / grammarVocab(grammarExamples.json jp) + vocabTokenize='jp'。
//  逐次保存・再開可(グループ単位)。実行: node scripts/build-vi-dict-jp-ctx.mjs [conv|grammar]
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const DATA = path.join(ROOT, 'expo-app/data');
const OVERLAY = path.join(DATA, 'overlays/vi.json');
...
async function analyze(items){
  ...
  const prompt=`You are a Japanese tokenizer + Japanese-Vietnamese lexicographer (for Vietnamese learners of Japanese).
...
- EVERY non-punctuation token MUST have a NON-EMPTY meaning. For grammatical tokens — auxiliaries/copula (です ます た ない), te-form connectives (て で), honorific prefixes (お ご), particles — give a SHORT functional Vietnamese gloss (e.g. ます=thể lịch sự, で=nối (thể te), お=tiền tố kính ngữ). Only punctuation may have meaning "".
For each token: "word"(exactly as written) / "reading"(romaji Hepburn) / "meaning"(VIETNAMESE, context-correct; particle/auxiliary=short functional gloss, never empty) / "pos" / "note"(Vietnamese or "").
...
async function main(){
  ...
  for(const kind of kinds){
    ...
    const dict=overlay[field]&&typeof overlay[field]==='object'?overlay[field]:{};
    // 既処理判定: contexts に当該sentence_idがあるか
    ...
    console.log(`\n=== ${kind} (${field}): ${sentences.length}文 / model=${MODEL} (ja→vi) ===`);
    let gdone=0;
    for(const g of groups){
      ...
      for(let i=0;i<gItems.length;i+=BATCH){
        ...
        catch(e){console.log('  [skip] '+batch[0].id+'.. 失敗: '+e.message);continue;}
  ...
  console.log(`\n辞書 全完了 / コスト ~$${totalCost.toFixed(2)} (約${Math.ceil(totalCost*150)}円)`);
}
