// App B ベンガル語パック 辞書(全件・最高品質): 日本語文を分解し各語にベンガル語の意味+読み+品詞+注記。
//  古い(ベンガル語キー)convVocab/grammarVocab を破棄し、日本語キーの分解辞書に作り直す。
//  出力: overlays/bn.json の convVocab/grammarVocab + vocabTokenize='jp'。逐次保存・再開可。
// 実行: node scripts/build-bn-dict-jp-ctx.mjs [conv|grammar]
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const DATA = path.join(ROOT, 'expo-app/data');
const OVERLAY = path.join(DATA, 'overlays/bn.json');
...
async function analyze(items){
  ...
  const prompt=`You are a Japanese tokenizer + Japanese-Bengali lexicographer (for Bengali learners of Japanese).
...
- EVERY non-punctuation token MUST have a NON-EMPTY meaning. For grammatical tokens — auxiliaries/copula (です ます た ない), te-form connectives (て で), honorific prefixes (お ご), particles — give a SHORT functional Bengali gloss (e.g. ます=ভদ্র রূপ, で=সংযোজক(তে-রূপ), お=সম্মানসূচক উপসর্গ). Only punctuation may have meaning "".
For each token: "word"(exactly as written) / "reading"(romaji Hepburn) / "meaning"(BENGALI, context-correct; particle/auxiliary=short functional gloss, never empty) / "pos" / "note"(Bengali or "").
...
async function main(){
  ...
  for(const kind of kinds){
    ...
    console.log(`\n=== ${kind} (${field}): ${sentences.length}文 / model=${MODEL} (ja→bn) ===`);
    let gdone=0;
